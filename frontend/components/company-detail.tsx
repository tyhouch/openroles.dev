...
import { X, ExternalLink, MapPin, Briefcase, Maximize2, Minimize2 } from 'lucide-react';
...
export function CompanyDetail({ company, summary, onClose }: CompanyDetailProps) {
  ...
  if (!company && !isClosing) return null;
  ...
  const functions = company ? [...new Set(company.jobs.map(j => j.function).filter(Boolean))] as string[] : [];

  // Filter jobs
  const filteredJobs = !company ? [] : (jobFilter === 'all'
    ? company.jobs
      ...
      >
        ...
        <div className="p-6 border-b border-border">
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            <MetricCard label="Open Roles" value={company?.active_jobs_count ?? 0} small />
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {activeTab === 'overview' ? (
            <div className={isExpanded ? 'grid grid-cols-2 gap-6' : ''}>
              <div>
                <div className="mb-6">
                ...
                {summary?.anomalies && summary.anomalies.length > 0 && (
                  <div className="mb-6">
                    <div className="label-caps mb-3 text-gold">Anomalies</div>
                    <ul className="space-y-2">
              ...
              {summary?.summary_text && (
                <div className={isExpanded ? '' : 'mt-0'}>
                  <div className="glass rounded-lg p-4">
                    ...
                    <p className="text-cream-muted text-[13px] leading-relaxed whitespace-pre-wrap">
                      {summary.summary_text}
                    </p>
                  </div>
                </div>
            ...
            <>
              ...
              {functions.length > 1 && (
                <div className="mb-4">
                  ...
                  >
                    <option value="all">All Functions ({company?.jobs.length ?? 0})</option>
                    {functions.map(fn => (
                      <option key={fn} value={fn}>
                        {fn} ({company?.jobs.filter(j => j.function === fn).length ?? 0})
                      </option>
                    ))}
                  </select>
                </div>
              ...
              {/* Jobs list */}
              <div className={`space-y-2 ${isExpanded ? 'grid grid-cols-2 gap-3 space-y-0' : ''}`}>
                {filteredJobs.map((job) => (
                  <JobCard key={job.id} job={job} />
                ))}
                {filteredJobs.length === 0 && (
                  <p className="text-cream-muted text-sm text-center py-8 col-span-2">
